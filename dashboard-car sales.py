...
#Plot 2 Calculate the average number of vehicles sold by vehicle type       
        # use groupby to create relevant data for plotting
        average_sales = recession_data.groupby('Year')['Automobile_Sales', 'Vehicle_Type'].mean().reset_index()
        R_chart2  = dcc.Graph(
...
# Plot 3 Pie chart for total expenditure share by vehicle type during recessions
        ...
        R_chart3 = dcc.Graph(
            figure=px.pie(exp_rec, 
                ...
                title="average number of vehicles sold by vehicle type {}".format(input_year)))

# Plot 4 bar chart for the effect of unemployment rate on vehicle type and sales
        unm_rec= recession_data.groupby('unemployment_rate')['Automobile_Sales'].mean().reset_index()
        R_chart3 = dcc.Graph(
            figure=px.bar(unm_data, 
                x='unemployment_rate', 
                y='Automobile_Sales', 
                title="effect of unemployment rate on vehicle type and sales {}".format(input_year)))
    ...
    # Total Advertisement Expenditure for each vehicle using pie chart
        ...
        Y_chart3 = dcc.Graph(
            figure=px.pie(avr_vdata, 
                ...
                title='Total Advertisement Expenditure for each vehicle using pie chart {}'.format(input_year)))
    ...
    # Plot bar chart for average number of vehicles sold during the given year
        ...
        Y_chart4 = dcc.Graph(
            figure=px.bar(exp_data, 
                x='Vehicle_Type', 
                y='Automobile_Sales', 
                title="average number of vehicles sold during the given year {}".format(input_year)))
#TASK 2.6: Returning the graphs for display
        return [
                html.Div(className='chart-item', children=[html.Div(R_chart1),html.Div(R_chart2)],style={'display': 'flex'}),
                html.Div(className='chart-item', children=[html.Div(R_chart3),html.Div(R_chart4)],style={'display': 'flex'})
